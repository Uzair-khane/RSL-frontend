  ...
  <div class="">
    <div class="booking-form w-full md:w-[400px]">
      <div class="grid grid-cols-2 w-full bg-[#2A2E31]">
        <button @click="toggleType('per_km')" :class="[
        ...
        <button @click="toggleType('hourly')" :class="[
      ...
      <form @submit.prevent="getFreeQuote" class="p-4 text-white space-y-3">
        <div>
          ...
          <input required v-model="booking.from_location" id="pickupLocation"
        ...
        <div v-if="type == 'per_km'">
          <label for="">To</label>
          <input required id="dropLocation" v-model="booking.drop_location"
            ...
            type="text" placeholder="Enter drop location" />
        </div>
        <div v-else>
          <label for="">Hourly</label>
          <select required v-model="booking.hours" @change="selectHours"
        ...
        <div class="relative w-full">
          ...
          <input v-model="booking.pickup_date" placeholder="dd / mm / yyyy" type="date" required
          ...
          <IconCalender class="fill-none pointer-events-none top-11 top-10 right-3 absolute stroke-[#0693E3] w-5 h-5" />
        </div>
        <div class="relative w-full">
          <label for="">Pickup Time</label>
          <input required v-model="booking.pickup_time"
            ...
            type="time" />
          <iconTime class="fill-none pointer-events-none top-11 top-10 right-3 absolute stroke-[#0693E3] w-6 h-5" />
        </div>
        <div class="w-full">
        ...
        <div>
          <div v-if="isBookingModal" class="fixed bg-[rgba(0,0,0,0.7)] inset-0"></div>
          <div :class="[
            isBookingModal
            ...
            <GoogleMap api-key="AIzaSyCc0FXkMRujB1PIFO39Z1Bg-Xu1AHGSlwE" :center="mapCenter" :zoom="12" ref="googleMap"
              class="w-full h-40">
            ...
            <ModalBooking @toggleModal="getFreeQuote" :booking="booking" :origin="origin" :destination="destination" />
          </div>
        </div>
      </form>
    </div>
